<template>
  <div class="max-w-2xl mx-auto">
    ...
    <div v-if="workerLoading" class="flex justify-center py-8"><div class="spinner"></div></div>

    <div v-else class="flex flex-col gap-5">
      <!-- Worker summary -->
      <div class="card flex items-center gap-4 worker-summary">
      ...
      <!-- Service type -->
      <div class="card">
        <label class="label">{{ t('booking.serviceType') }}</label>
        <select v-model="form.serviceType" class="input-field">
      ...
      <div class="card">
        ...
        <input v-model="form.address" type="text" class="input-field" :placeholder="t('booking.address')" />
        <label class="label mt-label">{{ t('booking.notes') }}</label>
        <textarea v-model="form.notes" class="input-field no-resize" rows="2"></textarea>
      </div>
      ...
      <div class="card">
        ...
        <div v-if="showAddPayment" class="modal-overlay" @click.self="showAddPayment = false">
          <div class="modal-box">
            <h3 class="card-title">{{ t('booking.addPaymentMethod') }}</h3>
      ...
      <button @click="handleBook" class="btn btn-primary btn-full btn-lg" :disabled="!canBook || submitting">
...
.summary-row { display:flex;justify-content:space-between;align-items:center;font-size:0.9375rem;margin-bottom:0.375rem; }
.modal-overlay { position:fixed;inset:0;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;z-index:1000;padding:1rem; }
.modal-box { background:white;border-radius:1rem;padding:1.5rem;width:100%;max-width:400px; }
.worker-summary { padding: 0.5rem; }
.avatar-blue { background:#2563eb; }
.avatar-initial { color:white; font-weight:700; }
.worker-name { font-weight:700; color:#1e293b; }
.worker-rate { font-size:0.875rem; color:#64748b; }
.card-subtitle { font-weight:600; color:#1e293b; margin-bottom:1rem; }
...
.mt-label { margin-top:0.875rem; }
.no-resize { resize: none; }
.muted-text { font-size:0.875rem; color:#64748b; margin-bottom:0.75rem; }
.pm-label { font-weight:600; font-size:0.9375rem; }
...
.total-label { font-weight:700; font-size:1.0625rem; }
